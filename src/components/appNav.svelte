<script lang="ts">
  ...
  let active = 'Inbox';
  let variant;
  $: variant = modal ? 'modal' : 'dismissable';
...
<!-- <Drawer {variant} class="mdc-top-app-bar--fixed-adjust" bind:open> -->
<Drawer {variant} bind:open class={!modal ? 'mdc-top-app-bar--fixed-adjust' : ''}>
  <Header>
